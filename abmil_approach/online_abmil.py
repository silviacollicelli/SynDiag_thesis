...
features_path = base_cfg['features_path']
labels_path = base_cfg['labels_path']
root_dir = base_cfg['root_dir']
annotations_file = base_cfg['annotations_file']
...
torch.manual_seed(seed)
model_freeze, optimizer_freeze, crit_freeze = build_model(False, config.l_rate, device)
torch.manual_seed(seed)
...
for epoch in range(config.epochs):     
    train_lf, train_af = train(model_freeze, device, crit_freeze, optimizer_freeze, train_dataloader)
    val_lf, val_af, stop = val(model_freeze, device, crit_freeze, val_dataloader, epoch, additional_metrics=False)
